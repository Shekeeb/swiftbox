...
const DriverDashboard = async () => {
  ...
  const totalDeliveries = driver ? await Order.countDocuments({
    driverId: (driver as any)._id,
    status: "delivered",
  })
    : 0

  const activeOrder = driver ? await Order.findOne({
    driverId: (driver as any)._id,
    status: { $in: ["assigned", "picked_up", "in_transit"] },
  }).lean()
    : null

  const recentOrders = driver ? await Order.find({
    driverId: (driver as any)._id,
  })
    .sort({ createdAt: -1 })
    .limit(5)
  ...
  const pendingOrders = driver ? await Order.find({
    status: "pending",
  ...
  const activeOrderData = activeOrder ? JSON.parse(JSON.stringify(activeOrder)) : null
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="w-full px-6 py-8 space-y-6">
        ...
        <DriverDashboardClient driverId={driverData?._id?.toString() || ""} isOnline={driverData?.isOnline || false} activeOrder={activeOrderData} userId={session.user.id} pendingOrders={pendingOrdersData}/>
